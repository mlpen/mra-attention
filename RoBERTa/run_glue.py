...
data_args = GlueDataTrainingArguments(
    ...
    max_seq_length = model_config["max_seq_len"], overwrite_cache = False)
...
optimizer = torch.optim.AdamW(
    model.parameters(),
    lr = args.lr,
    betas = (0.9, 0.999), eps = 1e-6, weight_decay = 0.02
)
...
log_file_name = f"glue-{args.task}-{args.checkpoint}-{args.lr}-{args.batch_size}-{args.epoch}-{time.time()}.log"
