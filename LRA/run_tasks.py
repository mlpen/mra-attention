from model import ModelForSC, ModelForSCDual
...
with open(os.path.join(checkpoint_dir, f"{task}_output-{init_t}-config.json"), "w") as config_f:
    json.dump(config, config_f, indent = 4)
    
log_f_path = os.path.join(checkpoint_dir, f"{task}_output-{init_t}.log")
