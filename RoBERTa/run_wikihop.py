...
wikihop_dataset_folder = os.path.join(curr_path, "datasets/wikihop")
...
checkpoint_dir = os.path.join(checkpoint_dir, f"wikihop-{args.checkpoint}-{args.lr}-{args.batch_size}-{args.epoch}-{time.time()}")
if not os.path.exists(checkpoint_dir):
    os.mkdir(checkpoint_dir)
    
with open(os.path.join(checkpoint_dir, "config.json"), "w") as config_f:
    json.dump(config, config_f, indent = 4)
    
########################### Running Model ###########################

log_file_name = f"output.log"
